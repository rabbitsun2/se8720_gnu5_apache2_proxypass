...
if (isset($_SESSION['ss_mb_id']) && $_SESSION['ss_mb_id']) { // 로그인중이라면
    ...
    if( ($member['mb_intercept_date'] && $member['mb_intercept_date'] <= date("Ymd", G5_SERVER_TIME)) 
        ...
        if (substr($member['mb_today_login'], 0, 10) != G5_TIME_YMD) {
            ...
            $sql = " update {$g5['member_table']} set mb_today_login = '".G5_TIME_YMDHIS."', mb_login_ip = '{$_SERVER['HTTP_X_FORWARDED_FOR']}' where mb_id = '{$member['mb_id']}' ";
            sql_query($sql);
        }
    }
} else {
...
if ($is_admin != 'super') {
    ...
    if ($cf_possible_ip) {
        ...
        for ($i=0; $i<count($pattern); $i++) {
            ...
            $pat = "/^{$pattern[$i]}$/";
            $is_possible_ip = preg_match($pat, $_SERVER['HTTP_X_FORWARDED_FOR']);
            if ($is_possible_ip)
                break;
        }
        if (!$is_possible_ip)
    ...
    for ($i=0; $i<count($pattern); $i++) {
        ...
        $pattern[$i] = str_replace("+", "[0-9\.]+", $pattern[$i]);
        $pat = "/^{$pattern[$i]}$/";
        $is_intercept_ip = preg_match($pat, $_SERVER['HTTP_X_FORWARDED_FOR']);
        if ($is_intercept_ip)
