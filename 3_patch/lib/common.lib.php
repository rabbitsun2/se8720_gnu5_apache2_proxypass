...
function get_uniqid()
{
    ...
    while (1) {
        ...
        $result = sql_query(" insert into {$g5['uniqid_table']} set uq_id = '$key', uq_ip = '{$_SERVER['HTTP_X_FORWARDED_FOR']}' ", false);
...
class html_process {
    ...
    public static function run()
    {
        ...
        // 현재접속자 처리
        $tmp_sql = " select count(*) as cnt from {$g5['login_table']} where lo_ip = '{$_SERVER['HTTP_X_FORWARDED_FOR']}' ";
        $tmp_row = sql_fetch($tmp_sql);
        $http_host = isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : $_SERVER['SERVER_NAME']; 

        if ($tmp_row['cnt']) {
            $tmp_sql = " update {$g5['login_table']} set mb_id = '{$member['mb_id']}', lo_datetime = '".G5_TIME_YMDHIS."', lo_location = '{$g5['lo_location']}', lo_url = '{$g5['lo_url']}' where lo_ip = '{$_SERVER['HTTP_X_FORWARDED_FOR']}' ";
            sql_query($tmp_sql, FALSE);
        } else {
            $tmp_sql = " insert into {$g5['login_table']} ( lo_ip, mb_id, lo_datetime, lo_location, lo_url ) values ( '{$_SERVER['HTTP_X_FORWARDED_FOR']}', '{$member['mb_id']}', '".G5_TIME_YMDHIS."', '{$g5['lo_location']}',  '{$g5['lo_url']}' ) ";
            sql_query($tmp_sql, FALSE);
...
function insert_cert_history($mb_id, $company, $method)
{
    ...
    $sql = " insert into {$g5['cert_history_table']}
                set mb_id = '$mb_id',
                    cr_company = '$company',
                    cr_method = '$method',
                    cr_ip = '{$_SERVER['HTTP_X_FORWARDED_FOR']}',
                    cr_date = '".G5_TIME_YMD."',
                    cr_time = '".G5_TIME_HIS."' ";
    sql_query($sql);
}
...
function certify_count_check($mb_id, $type)
{
    ...
    if($mb_id) {
        ...
        $sql .= " where cr_ip = '{$_SERVER['HTTP_X_FORWARDED_FOR']}' ";
    }
...
function replace_filename($name)
{
    ...
    $return_filename = sha1($ss_id.$_SERVER['HTTP_X_FORWARDED_FOR'].$usec); 
    if( $ext )
        $return_filename .= '.'.$ext;
...
function insert_popular($field, $str)
{
    ...
    if(!in_array('mb_id', $field)) {
        $sql = " insert into {$g5['popular_table']} set pp_word = '{$str}', pp_date = '".G5_TIME_YMD."', pp_ip = '{$_SERVER['HTTP_X_FORWARDED_FOR']}' ";
        sql_query($sql, FALSE);
    }
}
